...
public class DownResRule extends ResRule {

	public DownResRule(MetadataSetAttribute aAttribute,
	...
	public CMV apply(List<CMV> aCMVList) {
		if (itsFusionOperator == ResRule.VALUE)
			return new DownResCMV(itsValue,itsRestrictionOperator);
		if (itsFusionOperator == ResRule.MAX)
			return ((ResCMV)CMVUtils.maxCMV(aCMVList)).setNature(itsRestrictionOperator);
		if (itsFusionOperator == ResRule.MIN)
